...
nifti_local_moderated_t_stat = function(imgs, mask = NULL, nn, radius, adjust.method = "BH", ...){

  maps = nifti_images_to_matrix(imgs = imgs)
  nrow.map = nrow(maps)
  ...
  t.stat.list = parallel::mclapply(1:nrow.map, 
                     function(center){
                       ...
                       adj.pval = topTable(eb.fit, sort.by = 'none', adjust.method = adjust.method, number = length(idx))$adj.P.Val[1]
